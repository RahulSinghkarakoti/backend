...
const getAllVideos = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200,{},"fetch successfull")
  )
});
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  const video = await Video.findById(videoId);
  function extractPublicId(url) {  //function to fetch the publicId from a url 
    const regex = /\/upload\/[^\/]+\/([^\.]+)\./;
    const match = url.match(regex);
    return match ? match[1] : null;
  }
  console.log(extractPublicId(video.thumbnail))
  const delelteOldThumbnail = await deleteOnCloudinary(extractPublicId(video.thumbnail));
