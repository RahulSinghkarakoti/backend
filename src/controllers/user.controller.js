...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!avatar) throw new ApiError(500, "failed to uploade avatar");
  ...
  const createdUser = await User.findById(user._id).select(
    "-password " //these field are not selected
  );
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.findOne({
    $and: [{ username }, { email }], //find either username andd email and return the first entry that matches
  });
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    { new: true }
  ).select("-password"); //return the updated user
...
const updateAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatarLocalPath) throw new ApiError(400, "please provide a valid image");
